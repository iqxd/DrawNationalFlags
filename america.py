...
core.draw_rectangle(draw_id,[0,0],[star_rect_width,star_rect_height],blue_color,fill=blue_color)

for i in range(0,7):
    # draw_guide_line([star_rect_width,i*htile_len],[width,i*htile_len])
    color = white_color if i%2 else red_color
    core.draw_rectangle(draw_id,[star_rect_width,i*htile_len],[width,(i+1)*htile_len],color,fill=color)

for i in range(7,13):
    color = white_color if i%2 else red_color
    core.draw_rectangle(draw_id,[0,i*htile_len],[width,(i+1)*htile_len],color,fill=color)
...
for i in range(0,10):
    draw_guide_line([spad_width,spad_height+i*stile_height],[star_rect_width-spad_width,spad_height+i*stile_height])

for i in range(0,12):
    draw_guide_line([spad_width+i*stile_width,spad_height],[spad_width+i*stile_width,star_rect_height-spad_height]) 
